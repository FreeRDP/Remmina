...
static void remmina_ftp_client_popup_menu(RemminaFTPClient *client, GdkEventButton *event)
{
	...
	image = gtk_image_new_from_icon_name("remmina-document-save", GTK_ICON_SIZE_MENU);
	...
	gtk_widget_show(menuitem);
	image = gtk_image_new_from_icon_name("remmina-document-send", GTK_ICON_SIZE_MENU);
	gtk_widget_show(image);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
...
static GtkWidget* remmina_ftp_client_create_toolbar(RemminaFTPClient *client)
{
	...
	gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
	image = gtk_image_new_from_icon_name("remmina-document-save", GTK_ICON_SIZE_BUTTON);
