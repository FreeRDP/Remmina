	...
	else
	{
		...
		if (from != NULL && GTK_IS_NOTEBOOK(from))
		{
			...
			for (i = 0; i < n; i++)
			{
				...
				/* Reparent tc->viewport */
				g_object_ref(tc->viewport);
				gtk_container_remove(GTK_CONTAINER(gtk_widget_get_parent(tc->viewport)),tc->viewport);
				gtk_container_add(GTK_CONTAINER(tc->scrolled_container), tc->viewport );
				g_object_unref(tc->viewport);
			}
